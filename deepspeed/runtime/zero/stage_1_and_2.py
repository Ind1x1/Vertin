...
class DeepSpeedZeroOptimizer(ZeROOptimizer):
    ...
    def __init__(self,
        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            self.vertin_step_list.append(0)
    ...
    # TODO Important all-reduce main code
    # TODO run at acc boundy
    ...
    def _vertin_create_param_groups(self, param_groups):
        ...
        for i, group in enumerate(self.bit16_groups):
            single_grad_partition = torch.zeros(int(self.partition_size[i]),
                                                    dtype=self.single_partition_of_fp32_groups[i].dtype,
                                                    device='cpu')
            self.vertin_param_groups[i]['params'][0].grad = get_accelerator().pin_memory(
                    single_grad_partition)
        
        # TODO PROBLEM ATTENTION!!!! 这里的初始赋值还有点问题，后续解决
            # grad_copy = self.single_partition_of_fp32_groups[i].grad.detach().clone().cpu()
            # for param in self.vertin_param_groups[i]['params']:
    ...
    def _vertin_step(self, group_no):
        ...
        # for group_no, group in enumerate(self.vertin_optimizer.param_groups):
        #     for param in group['params']:
        #         param.grad = None
    ...
    def vertin_update(self, group_no):
        ...
        # 获取指定 key 的 basic_state 和 vertin_state
        basic_key = list(basic_state.keys())[group_no]
        vertin_key = list(vertin_state.keys())[group_no]

        # 获取 basic_state 和 vertin_state 对应 key 的 value
        basic_value = basic_state[basic_key]
        vertin_value = vertin_state[vertin_key]

        # 使用 vertin_value 中的 exp_avg 和 exp_avg_sq 替换 basic_value 中的 exp_avg 和 exp_avg_sq
        basic_value['exp_avg_sq'].copy_(vertin_value['exp_avg_sq'].to(basic_value['exp_avg_sq'].device))

        # 同样操作用于 exp_avg
        basic_value['exp_avg'].copy_(vertin_value['exp_avg'].to(basic_value['exp_avg'].device))
    ...
    def _optimizer_step(self, group_no):
        ...
        #    self.optimizer.step()
            ...
            self._vertin_step(group_no)

        self.vertin_step_list[group_no] = self.vertin_step_list[group_no] + 1
        if self.vertin_step_list[group_no] >= self.vertin_step_boundy:
            self.vertin_update(group_no)
            self.vertin_step_list[group_no] = 0
    ...
    def step(self, closure=None):
        ...
        for i, group in enumerate(self.bit16_groups):
            ...
            if self.cpu_offload:
                ...
                self.vertin_step_list[i] = self.vertin_step_list[i] + 1
                if self.vertin_step_list[i] == self.vertin_step_boundy:
                    self.vertin_update(i)
                    self.vertin_step_list[i] = 0
